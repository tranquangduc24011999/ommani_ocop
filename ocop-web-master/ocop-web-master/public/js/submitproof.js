...
const saveResult = (status, showModel, userId) => {
    ...
    for (let i = 0; i < questionsNameList.length; i++) {
        console.log(questionsNameList[i]);
        ...
        if ($(`#question input:checkbox[name="${questionsNameList[i]}"]:checked`).length < 1) {
            ...
            unansweredQuestions.push(questionsNameList[i])
            ...
            answerIdList.push($(`#question input:checkbox[name="${questionsNameList[i]}"]:checked`).data('answer-id'))
        }
    }
    //bôi màu và hiển thị danh sách nhung câu chưa được chọn
    if(unansweredQuestions.length>0){
        nonconformingSectionList=[];
        $incompleteAnswersMessage="Bạn chưa trả lời đầy đủ câu hỏi của mục ";
        $.each(unansweredQuestions,function (index,value){
            highlightedQuestions.push(value);
            $("span[name='"+value+"']").addClass('bg-danger');
            nonconformingSectionList.push($("span[name='"+value+"']").attr('value'));
        })
        $.each(highlightedQuestions,function (index,value){
            if (!unansweredQuestions.includes(value)){
                $("span[name='"+value+"']").removeClass('bg-danger');
            }
        })
        nonconformingSectionList=$.unique(nonconformingSectionList);
        return Swal.fire('Cảnh báo', `${$incompleteAnswersMessage + nonconformingSectionList.join(', ')}, vui lòng kiểm tra lại.`, 'warning');
    }
    starScore = parseInt($("#rankStar").text());
    //bôi màu và check những câu chưa tích đúng số sao so với điểm tổng
    $.each(answerIdList, function (index, value) {

        if(starScore==1){
            SuitableAnswerList=lv2
        }
        if(starScore==3){
            SuitableAnswerList=lv3
        }
        if(starScore==4){
            SuitableAnswerList=lv4
        }
        if(starScore==5){
            SuitableAnswerList=lv5
        }
        if (!SuitableAnswerList.includes(value)) {
            notValid.push(value)
        }
    })
    if ($(notValid).length > 0) {
        $.each(notValid, function (index, value) {
            isCheckedAgain.push(value)
            $('[data-answer-id="' + value + '"]').parent().addClass('bg-danger')
        })
        isCheckedAgain = $.unique(isCheckedAgain);
        $.each(isCheckedAgain, function (index2, value2) {
            if (!notValid.includes(value2)) {
                $('[data-answer-id="' + value2 + '"]').parent().removeClass('bg-danger')
            }
        })
        return Swal.fire('Cảnh báo', `Điểm của bạn đã được lưu lại. Các tiêu chí được bôi màu hiện đang chưa đạt hạng mức ${starScore} sao. Bạn có thể chấm và gửi lại`, 'warning').then(()=>{
            submitResult(userId);
        });
    }
    else {
...
function saveProof() {
    try {
        ...
        let data = $("#selectedqa").select2('data');
        var arrs = [];
        if (data.length > 0) {
            data.forEach((e) => {
                let id = e.id;
                        ...
                        document.getElementById('ImgModal-info').innerHTML += `
                            ...
                            <div class="card mb-4 shadow-sm">
                                ...
                                <div style="padding:10px">
                                        <a  href="##" class="btn btn-sm btn-outline-secondary" role="button" onclick="editQuote(\'${e.proof_file_id}\', \'${e.description}\')" style="margin-right: 11px;" >Sửa</a>
                                        ${deleteQuote}
                                </div>
...
const editQuote = (id, description) => {
    proofInfo = (id)
    $("#selected2qa").val([]).trigger("change");
    $('#step2').modal('toggle');
    $('#stepEdit').modal('show');
    $.ajax({
        url: "getfilebyid?id=" + id,
        type: "GET",
        success: function (response) {
            if (response) {
                document.getElementById('imgQueto').innerHTML = ``
                let link = `<img src="${response.data}" width="100px" height="100px" style="object-fit: cover;cursor: zoom-in;border-radius: 15px;" onclick="clickImg(\'${response.data}\')">`
                if (hasExtension(response.data, doc)) {
                    link = ` <a target="_blank" href="https://view.officeapps.live.com/op/view.aspx?src=${response.data}"><img src="images/word.jpg" style="object-fit: cover;cursor: zoom-in; width="100px"; height="100px";padding: 11px;border-radius: 17%;height: 121px;"></a> `
                } else if (hasExtension(response.data, excel)) {
                    link = ` <a target="_blank" href="https://view.officeapps.live.com/op/view.aspx?src=${response.data}"><img src="images/excel.png" style="object-fit: cover;cursor: zoom-in; width="100px"; height="100px";padding: 11px;border-radius: 17%;height: 121px;" ></a> `
                } else if (hasExtension(response.data, pdf)) {
                    link = ` <a target="_blank" href="${response.data}"><img src="images/pdf.jpg" style="object-fit: cover;cursor: zoom-in; width="100px"; height="100px";padding: 11px;border-radius: 17%;height: 121px;" ></a> `
                } else if (response.type == 'LINK') {
                    link = ` <a target="_blank" href="${response.data}">${response.data}</a> `
                }
                descriptionImgQuote.value = response.description
                document.getElementById('imgQueto').innerHTML = `${link}`
                let arrRe = []
                response.links.forEach((item) => {
                    //var index = arrQuote.map(function(e) { return e.id; }).indexOf(item.question_id);
                    //var position = index + 1;
                    //arrRe.push(position)
                    arrRe.push(item.question_id)
                })
                $('#selected2qa').val(arrRe);
                $('#selected2qa').trigger('change');
                buttonQuote.innerHTML = ``
                buttonQuote.innerHTML = `
                <a href="#" class="btn btn-secondary" data-dismiss="modal" style="    width: 122px;
                height: 40px;
                border-radius: 8px;
                background-color: #eff0f1;">Đóng</a>
                <button href="#" class="btn btn-success" onclick="savepoof(\'${response.id}\',\'${response.prinformation_id}'\)" style="    width: 140px;
                height: 40px;
                border-radius: 8px;
                background-color: #049252;" id="savePoofQuote">Lưu</button>
                `
...
const savepoof = (id, proof_information_id) => {
    let data = $("#selected2qa").select2('data');
    var arrs = [];
    if (data.length > 0) {
        data.forEach((e) => {
            let id = e.id;
    ...
    let objquote = {
        _token: _token,
        product_id: productId,
        proof_information_id: proof_information_id,
...
// }
$('#selected2qa').on('select2:unselect', function (e) {
...
function loadqa() {
    $('#selected2qa').val([]).trigger("change");
    $('#selected2qa').val(null).trigger("change");
    $('#selected2qa').val('').trigger("change");
    $('#selected2qa').empty()

    $.ajax({
        url: "getqa?product_type=" + productType,
        type: "GET",
        success: function (response) {
            if (response) {
                handleQA(response)
...
function handleQA(response) {
    ...
    arrQuote.forEach((data) => {
        let c = new Option(data.question, `${data.id}`);
        $('#selected2qa').append(c)
    })
...
function removeDuplicateIds(originArray) {
    let uniqueArray = [];
    originArray.forEach((c) => {
