  ...
  <div class="container-fluid">
        ...
        <div class="modal-content" style="margin-top: 9%;max-height: 723px;  background-color: #ffffff;">
          <div class="modal-body" style="padding: 0 24px;">
            ...
            <div class="form-group">
              ...
              <input onchange="searchMemberCouncil()" style="padding-left: 43px;" type="text" class="form-control input" id="seachmembercouncil"
                placeholder="Nhập email/số điện thoại">
            </div>
            <div class="row " style=" max-height: 453px;    overflow: auto;" id="listmembercouncil">

            </div>
            <div style="float: right;margin: 24px 0;">
              <a href="#" class="btn btn-secondary" data-dismiss="modal" onclick="">Hủy</a>
              <button href="#" class="btn btn-success" id="updateMemberToCouncils" onclick="updateMember()">Cập nhật</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="row" style="margin: 2px;">
      <div class="col-12" style="margin-bottom: 23px;">
        ...
        <a href="#" class="btn btn-sm btn-success" style="float: right;margin-right: 16px;display:none" data-toggle="modal"
    ...
    <div class="row" id="" style="    margin: 11px;">
      <div class="col-md-5">
        <div class="row" style="  background-color: #ffffff;width: 100%;">
          <div class="col-md-12" style="display: flex;padding: 24px;">
            <div class="rec"></div>
            <label class="SubHeading " id="memberTgv">THÀNH VIÊN HỘI ĐỒNG CHẤM</label>
      ...
      <div class="col-md-7" style="overflow: auto;max-height: 462px;">
        <div class="row" style="  background-color: #ffffff;width: 100%;">
          ...
          <div class="col-md-12" style="display: flex;padding: 24px;">
            ...
            <div style="    width: 112px;padding-top: 6px;    text-align: right;" id="idPro">
              <a href="#" class="Text" data-toggle="modal" data-target="#step3">
                <img src="{{ asset('images/s1.png') }}" class="Primary">
                <span class="text">Thêm mới</span>
              </a>
      ...
      <div class="col-md-12" style="overflow: auto;max-height: 462px;">
        <div class="row" style="  background-color: #ffffff;width: 100%;">

          <div class="col-md-12" style="display: flex;padding: 24px;">
            <div class="rec"></div>
            <label class="SubHeading ">COPY KẾT QUẢ </label>

          </div>

        </div>
        <form id="copy" action="javascript:void(0)" method="POST" enctype="multipart/form-data">
          <input type="hidden" name="_token" value="{!! csrf_token() !!}" />
        <div class="row " style=" background-color: #ffffff;" id="listproductcopy">
          <select class="custom-select" name="parent_id" id="parent_id">
            <option value="0">Chọn sản phẩm đã chấm xong</option>
            @foreach($products as $item)
            <option value="{!! $item->id !!}">{!! $item->name!!}</option>
            @endforeach
          </select>
          <br/>
          <select class="custom-select" name="product_id" id="product_id" style="margin-top: 15px">
            <option value="0">Chọn sản phẩm muốn copy dữ liệu của sản phẩm đã chấm</option>
            @foreach($products as $item)
            <option value="{!! $item->id !!}">{!! $item->name!!}</option>
            @endforeach
          </select>
          <br/>
          <button style="margin-top: 15px" type="submit" id="send_form" class="btn btn-success">Copy</button>
        </div>
      </form>
      </div>
    </div>
...
<script type="text/javascript">
  ...
  $('#copy').on('submit',function(event){
      event.preventDefault();
      var council_id = getParam('id');
      var _token = $("input[name='_token']").val();
      var selectorParent = document.getElementById('parent_id');
      var parent_id = selectorParent[selectorParent.selectedIndex].value;

      var selectorProduct = document.getElementById('product_id');
      var product_id = selectorProduct[selectorProduct.selectedIndex].value;

      $.ajax({
        url: "https://smartocop.com.vn/copymark",
        type:'POST',
        data: {_token:_token, council_id:council_id ,product_origin_id:parent_id, product_id:product_id},
